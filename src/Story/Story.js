...
class Story extends Component {
  ...
  gotoNode = nodeLink => this.setState(state => {
    ...
    if (nodeLink.type === NodeLinkTypes.INPUT && nodeLink.targetVariable) {
      ...
      newState.customData = { ...state.customData || {}, [targetVariable]:inputValue }
    }

    if (nodeLink.type === NodeLinkTypes.INVENTORY && nodeLink.item) {
      const { item, action } = nodeLink;
      let itemVal = state.inventory && state.inventory[item] ? state.inventory[item] : 0;
            ...
            <div className="story">
              {
                currentNode && (
                  <div className="node">
                    <NodeContent content={replaceVariables(currentNode.content, this.state.customData)} />
                    <div className="node-links">
                      { currentNode.next && currentNode.next.map(nl => React.createElement(nodeLinkTypes[nl.type] || NodeLink, { nodeLink:nl, onClick:this.gotoNode, key:btoa(`${nl.node}-${nl.content}`) })) }
                    </div>
                  </div>
                )
              }
