...
class Story extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (
      JSON.stringify(this.state.curentNode) !==
      JSON.stringify(nextState.currentNode)
    )
      return true;
    return false;
  }
  ...
  goToNode(nodeLink) {
    if (nodeLink.restart) {
      ...
      nodeLink.type === NodeLinkTypes.INPUT &&
      ...
      const newCount =
        this.state.inventory && this.state.inventory[nodeLink.item]
          ? this.state.inventory[nodeLink.item] + inc
          : inc;
      this.setState(
        {
          ...this.state,
  ...
  render() {
    ...
    return (
      <div className="story">
        <div className="node">
          ...
          <div className="node-links">
            {currentNode.next && currentNode.next.length ? (
              currentNode.next.map(n =>
                React.createElement(nodeLinkTypes[n.type] || NodeLink, {
                  nodeLink: n,
                  nodeLinkClicked: this.goToNode,
                  key: btoa(`${n.node}-${n.content}`)
                })
              )
            ) : (
        ...
        <Debug>{() => <pre>{JSON.stringify(currentNode, null, 1)}</pre>}</Debug>
      </div>
    );
  }
}
